...
import {setField, saveTask, setAllFieldsTasks, resetForm} from '../actions';
import '../util/pt-br';
...
class NewTask extends Component {
  ...
  componentDidMount() {
    const {navigation, setAllFieldsTasks, resetForm} = this.props;
    const {params} = navigation.state;
    if (params.taskToEdit && params) {
      setAllFieldsTasks(params.taskToEdit);
      this.setState({title: 'Editar'});
    } else {
      this.setState({title: 'Criar Nova'});
      resetForm();
    }
  }

  render() {
    const {taskForm, setField, saveTask, navigation} = this.props;
      ...
      <View style={Styles.container}>
        <ScrollView
          ...
          <View style={Styles.titleContainer}>
            <Text style={Styles.projectTitle}>{this.state.title} Tarefa</Text>
          </View>
          <View style={Styles.reqContainer}>
            ...
            <TextInput
              value={taskForm.req}
              onChangeText={(value) => setField('req', value)}
              style={styles.input}
              multiline
            ...
            <View>
              <TouchableOpacity
                ...
                <View style={[styles.row, styles.date]}>
                  ...
                  <Text>{taskForm.date || this.state.date}</Text>
                </View>
              </TouchableOpacity>
              <DateTimePicker
